...
void nuklear_build_ui(void)
{
    ...
    if (nk_begin(g_ctx, "Property Panel", s_property_panel_bounds,
                 NK_WINDOW_BORDER)) {
        s_property_panel_bounds = nk_window_get_bounds(g_ctx);
        ...
        if (sel && sel_count(sel) > 0) {
            ...
            nk_layout_row_dynamic(g_ctx, 20, 1);
            nk_label(g_ctx, first->vtable->name, NK_TEXT_LEFT);
            ...
            nk_slider_float(g_ctx, 0.0f, &r, 1.0f, 0.01f);
            ...
            nk_slider_float(g_ctx, 0.0f, &g, 1.0f, 0.01f);
            for (int i = 0; i < sel_count(sel); i++) {
            ...
            nk_slider_float(g_ctx, 0.0f, &b, 1.0f, 0.01f);
            for (int i = 0; i < sel_count(sel); i++) {
            ...
            nk_slider_float(g_ctx, 0.0f, &a, 1.0f, 0.01f);
            for (int i = 0; i < sel_count(sel); i++) {
            ...
            nk_layout_row_dynamic(g_ctx, 20, 1);
            nk_label(g_ctx, "Line Width:", NK_TEXT_LEFT);

            float lw = 1.0f;
            shape_get_property(first, "line_width", &lw);
            nk_slider_float(g_ctx, 1.0f, &lw, 10.0f, 0.1f);
            for (int i = 0; i < sel_count(sel); i++) {
...
int nuklear_ui_blocks_mouse_input(double x, double y)
{
    ...
    if (nk_item_is_any_active(g_ctx)) {
        return 1;
    }
